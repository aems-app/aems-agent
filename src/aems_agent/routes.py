...
# new platform requires a code review. macOS ships the .dmg that the build
# workflow produces (see .github/workflows/build.yml — "Build DMG from signed
# .app"); the DMG root contains "AEMS Agent.app", "com.aems.agent.plist", and
# an "/Applications" symlink, so the relaunch script mounts → replaces the
# .app in /Applications → detaches → re-opens.
_SELF_UPDATE_ASSET_BY_PLATFORM: Dict[str, str] = {
    "win32": "aems-agent-setup.exe",
    "darwin": "AEMS-Agent.dmg",
}

# The .app bundle name inside the DMG root. Pulled out as a constant so the
# macOS relaunch script and its test stay in sync if the build ever renames it.
_MACOS_APP_BUNDLE_NAME = "AEMS Agent.app"
...
def _spawn_installer_detached(installer_path: Path) -> int:
    """Spawn the platform installer fully detached. Returns the child PID.

    Windows: launches the NSIS ``aems-agent-setup.exe /S`` with
    ``DETACHED_PROCESS | CREATE_NEW_PROCESS_GROUP`` so the installer's
    own ``taskkill`` step can take down THIS process without orphaning
    the installer.

    macOS: builds a short relaunch shell script that:
      1. waits ~2 s so the HTTP response reaches the browser,
      2. quits the currently-running ``AEMS Agent``,
      3. mounts the DMG read-only via ``hdiutil attach``,
      4. replaces ``/Applications/AEMS Agent.app`` from the DMG via ``cp -R``,
      5. detaches the DMG,
      6. re-opens the new ``AEMS Agent.app`` via ``open -a``.
    The script is spawned via ``Popen(..., start_new_session=True)`` so
    SIGTERM to this process does not cascade into the relaunch script.

    Linux: not implemented — caller is gated by
    ``_SELF_UPDATE_ASSET_BY_PLATFORM`` which omits the linux entry.
    """
    import subprocess
    ...
    if sys.platform == "darwin":
        return _spawn_macos_relaunch_script(installer_path)

    raise RuntimeError(  # pragma: no cover — guarded at call site
        f"detached installer spawn not implemented for {sys.platform!r}"
    )


def _build_macos_relaunch_script(dmg_path: Path) -> str:
    """Return the bash that the macOS relaunch helper runs.

    Pulled out so the test can assert on the exact command shape without
    rebuilding the path-quoting logic.
    """
    import shlex

    dmg_quoted = shlex.quote(str(dmg_path))
    bundle_quoted = shlex.quote(_MACOS_APP_BUNDLE_NAME)
    # Mount under a per-run temp dir so we never collide with another mount.
    mount_dir = "$(mktemp -d /tmp/aems-self-update-XXXXXX)"

    return (
        "#!/bin/bash\n"
        "# AEMS Agent self-update relaunch helper. Generated by\n"
        "# aems_agent.routes._build_macos_relaunch_script.\n"
        "set -u\n"
        f"DMG={dmg_quoted}\n"
        f"BUNDLE={bundle_quoted}\n"
        f"MOUNT={mount_dir}\n"
        'APP_DST="/Applications/$BUNDLE"\n'
        "# 1. Let the HTTP response flush before we kill the agent.\n"
        "sleep 2\n"
        "# 2. Quit the currently-running tray. Try the friendly path first,\n"
        "#    fall back to pkill so we still proceed if AppleScript is\n"
        "#    blocked by the user's Automation privacy setting.\n"
        "osascript -e 'tell application \"AEMS Agent\" to quit' >/dev/null 2>&1 || true\n"
        "sleep 1\n"
        "pkill -f aems-agent >/dev/null 2>&1 || true\n"
        "sleep 1\n"
        ...
        "# 6. Relaunch the new tray.\n"
        'open -a "$APP_DST"\n'
    )


def _spawn_macos_relaunch_script(dmg_path: Path) -> int:
    """Write the relaunch helper to a tempfile and spawn it detached."""
    import os
    import stat
    import subprocess

    script_path = dmg_path.parent / "aems-agent-self-update-relaunch.sh"
    script_path.write_text(_build_macos_relaunch_script(dmg_path), encoding="utf-8")
    # chmod 0o700 — owner-only execute is sufficient since the script
    # lives in a per-user tempdir.
    os.chmod(script_path, stat.S_IRWXU)

    p = subprocess.Popen(  # noqa: S603 — script we just wrote in our own tempdir
